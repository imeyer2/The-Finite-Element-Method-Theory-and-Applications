...
def set_dirichlet(p, A, b, boundary_value : float = 0.0) -> tuple[np.ndarray, np.ndarray]:
    ...
    -------
    tuple
        (A_bc, b_bc) - modified matrix and vector with Dirichlet boundary conditions applied
...
def solve_advection_diffusion(
    ...
    for k in tqdm(range(steps+1), desc="Time stepping"):
        ...
        LHS = M/dt + D + C
        RHS = M.dot(u)/dt
        LHS_bc, RHS_bc = set_dirichlet(p, LHS.copy(), RHS.copy())
...
if __name__=="__main__":
    solve_advection_diffusion(
      ...
      steady_state=False, dt=0.01, T_final=2.0
    )
